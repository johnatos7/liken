...
public class QuestionInflaterforAnswersPage {
	...
	Activity activity;
	public QuestionInflaterforAnswersPage(LinearLayout ll_MediaView, TextView tv_Question,
...
/*	hmData.put("QuestionID", object.getString("QuestionID"));
	...
	public void inflate(){
		InflateTextContent();
		if(tempData.get("q_Media_Type").equals("1")){
			generateMediaView();
		}else if(tempData.get("q_Media_Type").equals("2")){
			processURL(2);
		}else if(tempData.get("q_Media_Type").equals("3")){
			processURL(3);
		}else if(tempData.get("q_Media_Type").equals("0")){
	...
	public void processURL(int Type)  {	
			if(Type==2){
			JsoupTask jsoupTask= new JsoupTask(tempData.get("q_Media_Link"));
			...
			SimpleYouTubeHelper youtubehelper=new SimpleYouTubeHelper(tempData.get("q_Media_Link"));
	...
	//	ll_MediaView.removeAllViewsInLayout();
		...
		s3MediaUtil.S3GetImageUri(tempData.get("q_Media_Link"));
	...
	private void viewInBrowser() {
		// Display in Browser.
		Uri uri =  Uri.parse(tempData.get("q_Media_Link") );
